...
function convertCase(anythingIwant) {
  if (anythingIwant === 'paper') return 'Papel';
  if (anythingIwant === 'scissors') return 'Tesoura';
  return 'Pedra';
}
...
function win(user, computer) {
  ...
  const userName = ' (Usuário)'.fontsize(3).sup();
  const compName = ' (Computador)'.fontsize(3).sup();
  result_div.innerHTML = `<p>${convertCase(user)}${userName} vence ${convertCase(computer)}${compName}. Você Ganhou!</p>`;
...
function loses(user, computer) {
  ...
  const userName = ' (Usuário)'.fontsize(3).sup();
  const compName = ' (Computador)'.fontsize(3).sup();
  result_div.innerHTML = `<p>${convertCase(computer)}${compName} vence ${convertCase(user)}${userName}. Você Perdeu!</p>`;
  const roundStatus = document.getElementById(user);
...
function draw(user, computer) {
	const userName = ' (Usuário)'.fontsize(3).sup();
  const compName = ' (Computador)'.fontsize(3).sup();
  result_div.innerHTML = `<p>Foi um empate! Vocês dois escolheram ${convertCase(user)}</p>`;
